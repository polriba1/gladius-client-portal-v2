...
const Landing = () => {
  ...
  useEffect(() => {
    ...
    const effectiveConversionRate = Number.isFinite(conversionRate) ? conversionRate : 30;
                ...
                <div className="grid md:grid-cols-2 gap-8 text-center">
                  <div>
                    <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-2">
                      {t('landing.calculator.recoveryWithGladius')} ({conversionRate}%)
                    </h4>
                    <p className="text-3xl font-bold text-primary">
                      {new Intl.NumberFormat(language === 'ca' ? 'ca-ES' : 'es-ES', { style: 'currency', currency: 'EUR' }).format(monthlyRecovery)}
                    </p>
                  </div>
                  <div>
                    <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-2">
                    ...
                    <p className="text-3xl font-bold text-primary">
                      {new Intl.NumberFormat(language === 'ca' ? 'ca-ES' : 'es-ES', { style: 'currency', currency: 'EUR' }).format(yearlyBenefit)}
                    </p>
                  </div>
                </div>
              </div>
